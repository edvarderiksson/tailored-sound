...
def get_mood_songs(query):
	...
	for playlist in range(0,len(data['playlists']['items'])):
		...
		for track in range(0,len(track_data['items'])):
				if track_data['items'][song_num]['track']['name'] in all_songs:
					# current_value = all_songs[track_data['items'][song_num]['track']['uri']]
					current_value = all_songs[track_data['items'][song_num]['track']['name']]
					current_value += 1
					# all_songs[track_data['items'][song_num]['track']['uri']] = current_value
					all_songs[track_data['items'][song_num]['track']['name']] = current_value
				else:
					# all_songs[track_data['items'][song_num]['track']['uri']] = 1
					all_songs[track_data['items'][song_num]['track']['name']] = 1
				song_num += 1

		sort_ed = srt.get_fifty(all_songs)
...
def get_mood_songs(query, exclude):
	...
	print(data['playlists']['items'][0]['tracks'].encode("utf-8"))
